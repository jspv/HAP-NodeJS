...
Accessory.prototype._handleSetCharacteristics = function(data, events, callback, remote, connectionID) {
  ...
  data.forEach(function(characteristicData) {
    ...
    // JSP - Hack to work around current iOS bug where MotionDetections and OccupancyDetection are being unregistered
    if ((characteristic.displayName == "Motion Detected" || characteristic.displayName == "Occupancy Detected") && ev === false) {
      debug('[%s] **** Ignoring Unresigster Request for "%s"', this.displayName, characteristic.displayName);
      // set ev to undefined
      ev = undefined;
    }
